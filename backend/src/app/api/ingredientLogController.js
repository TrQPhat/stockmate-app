const { Ingredientlog } = require("../models");

class ingredientLogController {
  ...
  async getProductLogById(req, res) {
    try {
      const { id } = req.params;
      const log = await Ingredientlog.findByPk(id);
      if (!log) {
  ...
  async createProductLog(req, res) {
    try {
      ...
      const newLog = await Ingredientlog.create({
        id,
        product_id,
        action,
        quantity,
...
module.exports = new ingredientLogController();
